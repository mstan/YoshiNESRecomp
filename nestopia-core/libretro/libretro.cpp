...
/* ---- PPU state accessors for recomp bridge ---- */
#include "../source/core/NstMachine.hpp"

extern "C" {

void nestopia_get_ppu_chr(uint8_t *out, int size) {
...
void nestopia_get_ppu_palette(uint8_t *out) {
    Core::Machine& m = emulator;
    memcpy(out, m.ppu.GetPaletteRam(), 0x20);
}

void nestopia_get_ppu_oam(uint8_t *out) {
    Core::Machine& m = emulator;
    memcpy(out, m.ppu.GetOamRam(), 0x100);
}
