<#
make_release.ps1 -- build the Windows release zip for YoshiNESRecomp.

Ships ONE windows zip (never a bare exe -- the exe needs SDL2.dll and the
launcher/ assets):

  YoshiRecomp-windows-x64.zip
      YoshiRecomp.exe + SDL2.dll + keybinds.ini + launcher/ + launcher-deps/ + README.txt
...
Publish AFTER smoke-testing the zip from a scratch directory:
  ...
  gh release create vX.Y.Z release\YoshiRecomp-windows-x64.zip `
...
$exe = Join-Path $bin 'YoshiRecomp.exe'
if (-not (Test-Path $exe)) { throw "missing $exe -- build the game first (build_all.bat, or cmake --build build_release)" }

$readme = @'
Yoshi - Static Recompilation
================================

A native PC build of Yoshi, produced by statically recompiling the NES
ROM's 6502 code to C with the NESRecomp framework (github.com/mstan/nesrecomp).

No ROM is included. On first launch, select your legally-obtained Yoshi
...
$zip = Join-Path $out 'YoshiRecomp-windows-x64.zip'
